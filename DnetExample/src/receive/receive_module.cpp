...
void AreaPickingCallback(const pcl::visualization::AreaPickingEvent& event, void *args){
	pcl::PointCloud<PointCfans>::Ptr clustercloud(new pcl::PointCloud<PointCfans>);
	...
	for (size_t i = 0; i < indices.size(); i++) {
		select_cloud_->points.push_back(show_cloud_g->points.at(indices[i]));
	}
	float meanx = 0;
	float meany = 0;
	float meanz = 0;
	float num = 0;
	for(auto p: select_cloud_->points){
		meanx += p.x;
		meany += p.y;
		meanz += p.z;
		num++;
	...
	float range = sqrt(meanx*meanx + meany*meany);

	printf("\033[40;31m [INFO] Calculate range : %f \033[0m \n", range);
}
...
void ReceiveModule::onInit()
{
	...
	mPlatformSteeringCommand.trans_vel = 0.0;
	mPlatformSteeringCommand.curvature = 0.0;	
}
